<?php
  ...
  // include "session_timeout.php";
    ...
    if ($type == "password") {
        $sql0 = "SELECT * FROM Users WHERE cust_id=".$id." AND pwd='".$old_pwd."'";
        $info_string = "Password";
    }

    if ($type == "pin") {
        $sql0 = "SELECT * FROM Users WHERE cust_id=".$id." AND pin='".$old_pwd."'";
        $info_string = "PIN";
    }
    ...
    if (($result0->num_rows) > 0) {
        if ($new_pwd == $check_pwd) {
            if ($new_pwd != $old_pwd) {
                if ($type == "password") {
                    $sql1 = "UPDATE Users SET pwd = '$new_pwd' WHERE cust_id=".$id;
                }
                if ($type == "pin") {
                    $sql1 = "UPDATE Users SET pin = '$new_pwd' WHERE cust_id=".$id;
                }
...
<body>
    <div class="flex-container">
        ...
        <div class="flex-item">
            <a href="/Users_profile.php" class="button">Go Back</a>
        </div>
    </div>
